...
events_genes_annotation <- merge(unique_genes,events_annotation,by.x="gene_id",by.y="V2")
...
rownames(results) <- as.character(events_annotation_filtered$V3)
PSI1_filtered <- PSI1[which(PSI1$event_id%in%as.character(events_annotation_filtered$V3)),]
...
rownames(results) <- as.character(events_annotation_filtered$V3)
PSI2_filtered <- PSI2[which(PSI2$event_id%in%as.character(events_annotation_filtered$V3)),]
...
rownames(results) <- as.character(events_annotation_filtered$V3)
PSI3_filtered <- PSI3[which(PSI3$event_id%in%as.character(events_annotation_filtered$V3)),]
...
while(cont<318){
  ...
  # If the selected event is already in the positive set, generate a new one
  if(as.character(PSI_all4_A5_A3[random,1]) %in% as.character(positives$event_id)){
    next   
  }
...
while(cont<277){
  # print(cont)
  random <- sample(1:nrow(PSI_all4_SE), 1, replace=FALSE)
  # If the selected event is already in the positive set, generate a new one
  if(as.character(PSI_all4_SE[random,1]) %in% as.character(positives$event_id)){
    next   
  }
  #If dont, add it to the negative
  sampling2 <- c(sampling2,random)
...
negatives <- PSI_all4_SE[sampling2,]
